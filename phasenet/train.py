...
def read_args():
    ...
    parser.add_argument("--log_dir", default="model", help="Log directory (default: log)")
...
def train_fn(args, data_reader, data_reader_valid):
    ...
    # Merge các summary để ghi tensorboard
    model.summary_op_train = model.summary_train
    model.summary_op_valid = model.summary_valid
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        for epoch in range(args.epochs):
            ...
            for _ in progressbar:
                           # Tính metrics sau mỗi epoch (dùng để in và log file)
                acc = sess.run(model.accuracy, feed_dict={model.drop_rate: 0, model.is_training: False})
                ...
                progressbar.set_description("{}: epoch {}, acc={:.4f}, loss={:.6f}, mean={:.6f},".format(log_dir.split("/")[-1], epoch, acc, loss_batch, train_loss.value))
 

            flog.write("epoch: {}, acc: {:.4f}, loss: {:.6f}, mean loss: {}\n".format(epoch, acc, loss_batch, train_loss.value))
            # Ghi tensorboard train
            summary_train = sess.run(model.summary_op_train, feed_dict={model.drop_rate: 0, model.is_training: False})
            summary_writer.add_summary(summary_train, sess.run(model.global_step))
            ...
            if data_reader_valid is not None:
                ...
                # Ghi tensorboard valid
                summary_valid = sess.run(model.summary_op_valid, feed_dict={model.drop_rate: 0, model.is_training: False})
                summary_writer.add_summary(summary_valid, sess.run(model.global_step))

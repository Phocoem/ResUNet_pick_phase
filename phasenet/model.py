...
class UNet:
  ...
  def add_metrics_op(self):
    with tf.compat.v1.variable_scope("metrics"):
        ...
        # Log train
        self.summary_train.extend([
            tf.compat.v1.summary.scalar("train_precision_p", precision_P),
            tf.compat.v1.summary.scalar("train_recall_p", recall_P),
            tf.compat.v1.summary.scalar("train_f1_p", f1_P),
            tf.compat.v1.summary.scalar("train_precision_s", precision_S),
            tf.compat.v1.summary.scalar("train_recall_s", recall_S),
            tf.compat.v1.summary.scalar("train_f1_s", f1_S),
        ])

        # Log valid
        self.summary_valid.extend([
            tf.compat.v1.summary.scalar("valid_precision_p", precision_P),
            tf.compat.v1.summary.scalar("valid_recall_p", recall_P),
            tf.compat.v1.summary.scalar("valid_f1_p", f1_P),
            tf.compat.v1.summary.scalar("valid_precision_s", precision_S),
            tf.compat.v1.summary.scalar("valid_recall_s", recall_S),
            tf.compat.v1.summary.scalar("valid_f1_s", f1_S),
        ])

        # Lưu tensor nếu cần dùng sau

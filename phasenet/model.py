...
class UNet:
  def __init__(self, config=ModelConfig(), input_batch=None, mode='train'):
    ...
    if self.weight_decay > 0:
        ...
        self.regularizer = tf.keras.regularizers.l2(0.5 * weight_decay)
    else:
  ...
  def res_block(self, x, filters, name):
      with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
          shortcut = x
          net = tf.compat.v1.layers.conv2d(x, filters=filters, kernel_size=self.kernel_size,
                                            padding='same', use_bias=False,
                                            kernel_initializer=self.initializer,
                                            kernel_regularizer=self.regularizer, name='conv1')
          net = tf.compat.v1.layers.batch_normalization(net, training=self.is_training, name='bn1')
          net = tf.nn.relu(net)
          ...
          net = tf.compat.v1.layers.batch_normalization(net, training=self.is_training, name='bn2')

          if shortcut.get_shape().as_list()[-1] != filters:
              shortcut = tf.compat.v1.layers.conv2d(shortcut, filters=filters, kernel_size=(1, 1),
                                                    padding='same', use_bias=False,
                                                    kernel_initializer=self.initializer,
                                                    kernel_regularizer=self.regularizer, name='conv_proj')
              shortcut = tf.compat.v1.layers.batch_normalization(shortcut, training=self.is_training, name='bn_proj')

          net += shortcut
          return tf.nn.relu(net)

  def add_prediction_op(self):
      # input conv replaced by res_block
      with tf.compat.v1.variable_scope("Input"):
          net = self.res_block(self.X, filters=self.filters_root, name="res_input")
      ...
      for depth in range(0, self.depths):
          with tf.compat.v1.variable_scope("DownConv_%d" % depth):
              ...
              net = self.res_block(net, filters=filters, name="res_down_%d" % (depth + 1))
              convs[depth] = net
      ...
      for depth in range(self.depths - 2, -1, -1):
          with tf.compat.v1.variable_scope("UpConv_%d" % depth):
              ...
              net = self.res_block(net, filters=filters, name="res_up_%d" % (depth + 1))
  ...
  def add_metrics_op(self):
    with tf.compat.v1.variable_scope("metrics"):
      preds_clean = tf.squeeze(self.preds, axis=2) if self.preds.get_shape().ndims == 4 else self.preds
      Y = tf.argmax(self.Y, axis=-1)
      preds_cls = tf.argmax(preds_clean, axis=-1)
      Y= tf.argmax(input=self.Y, axis=-1)
      confusion_matrix = tf.cast(tf.math.confusion_matrix(
          labels=tf.reshape(Y, [-1]),
          predictions=tf.reshape(preds_cls, [-1]),
          num_classes=self.n_class,
          name='confusion_matrix'),
          dtype=tf.float32)
      correct = tf.equal(tf.argmax(self.preds, axis=-1), tf.argmax(self.Y, axis=-1))
      self.accuracy = tf.reduce_mean(tf.cast(correct, tf.float32))

      # Log cho TensorBoard nếu muốn
      self.summary_train.append(tf.compat.v1.summary.scalar("train_accuracy", self.accuracy))
      self.summary_valid.append(tf.compat.v1.summary.scalar("valid_accuracy", self.accuracy))
